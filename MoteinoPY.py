...
class MoteinoNetwork(object):
    ...
    def __init__(self,
        ...
        self.Serial = MySerial(port=port,
                               baudrate=115200)
        self.Serial.write('X')     # send reset sign
        time.sleep(0.6)  # sleep  for 0.6 seconds, bootloader uses 0.5 seconds
        incoming = self.Serial.readline().rstrip()
        logging.debug("waiting for waking up sign...")
        assert incoming == b"moteinopy basesketch v2.2"
        logging.debug("... got it, base with " + incoming + " seems to be present")
        encryption_key_hex = ""
        for x in encryption_key:
            encryption_key_hex += _hexprints(x)
        self.Serial.write(_hexprints(frequency) +
                          _hexprints(base_id) +
                          _hexprints(network_id) +
                          _hexprints(high_power) +
                          encryption_key_hex + "\n")
        logging.debug("waiting for ready sign")
        incoming = self.Serial.readline().rstrip()
    ...
    def send2radio(self, send2id, payload, max_wait=None):
        ...
        with self.SerialLock:
            self.Serial.write(str(_hexprints(send2id) + payload + '\n').encode('ascii'))
