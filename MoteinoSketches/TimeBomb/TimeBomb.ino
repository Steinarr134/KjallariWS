...
Method and circuit updated (kbf), 03.01.18:
- Instead of solinoid actuated vape, smoke is now generated by a dedicated smoke machine. 
  Inside the machine is a transistor driven relay. 3 wires come out: red 5v, black gnd and yellow/color signal,
  so it can be triggered by 3.3v.
  Transistor triggered by 3.3v "HIGH", switches 5v, triggers relay, switches 230v.

  smoke machine Circuit details: 
  - BC337 npn transistor, ground side of relay (either way is fine).
  - current limiting resistor (~300 to ~1k, non-critical) on npn base.
  - pull-down resistor (~ 100k) from npn base to GND
  - 5v relay, "normally open"
  - 1N4001 "flyback" diode across relay 5v terminals
