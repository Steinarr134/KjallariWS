...
moteino = MoteinoNetwork("COM6", base_id=55, init_base=False)
moteino.logger.setLevel(logging.DEBUG)
...
while True:
    ...
    adcRead = 50  # adcRead = mcp.read_adc(0) / 4
    counter += 1
    amplitude = adcRead
    #----------BUTTONS---------------------------#
    #if (GPIO.input(16) == False):			#BUTTON 3
	# Unused function
    if False: #(GPIO.input(20) == False):			#BUTTON 2
        amplitude = random.randrange(200, 255, 10)
    if False: #(GPIO.input(21) == False):			#BUTTON 1
    ...
    if False: #(GPIO.input(12) == True):                           # Handprint not present
        amplitude = 0
        flatline = 1
